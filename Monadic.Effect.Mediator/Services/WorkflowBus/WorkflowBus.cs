    ...
    : IWorkflowBus
{
    ...
    /// from Monadic.Effect rather than the base Monadic implementation.
    /// </remarks>
    public Task<TOut> RunAsync<TOut>(object workflowInput, Metadata? metadata = null)
    {
        ...
        var runMethod = workflowService
            ...
            // Run(input) has an implementation in Monadic and Monadic.Effect (the latter with the "new" keyword).
            // We want the one from Monadic.Effect
