...
namespace Monadic.Effect.Data.Postgres.Utils;

/// <summary>
/// Provides functionality for migrating the PostgreSQL database schema for the Monadic.Effect.Data.Postgres system.
/// </summary>
/// <remarks>
/// The DatabaseMigrator class is responsible for ensuring that the PostgreSQL database schema
/// is up-to-date with the latest version of the Monadic.Effect.Data.Postgres system.
///
/// This class:
...
public class DatabaseMigrator
{
    ...
    public static async Task Migrate(string connectionString)
    {
        ...
        {
            Console.WriteLine(
                $"Caught Exception ({e.GetType()}) while attempting to migrate Monadic database: {e}"
            );
            throw;
        }
    }
