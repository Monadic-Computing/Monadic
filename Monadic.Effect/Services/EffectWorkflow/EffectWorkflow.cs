...
public abstract class EffectWorkflow<TIn, TOut> : Workflow<TIn, TOut>, IEffectWorkflow<TIn, TOut>
{
    ...
    /// in the service collection using AddMonadicEffects().
    /// </remarks>
    [Inject]
    ...
    public new virtual async Task<TOut> Run(TIn input)
    {
        if (EffectRunner == null)
        {
            EffectLogger?.LogCritical(
                "EffectRunner is null. Ensure services.AddMonadicEffects() is being added to your Dependency Injection Container."
            );
    ...
    private async Task<Metadata> InitializeWorkflow(TIn input)
    {
        if (EffectRunner == null)
            throw new WorkflowException(
                "EffectLogger is null. Something has gone horribly wrong. Ensure services.AddMonadicEffects() is being added to your Dependency Injection Container."
            );
    ...
    private async Task<Unit> FinishWorkflow(Either<Exception, TOut> result)
    {
        if (Metadata == null)
            throw new WorkflowException(
                "Metadata object has not been set. Was Run() called initially? If so, please submit a ticket on the Monadic GitHub repository. Something has gone horribly wrong."
            );
